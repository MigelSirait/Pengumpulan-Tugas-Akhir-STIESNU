  ...
  <div class="container-fluid home-page-container">
    <div class="row mb-3">
      <div class="col-12 d-flex justify-content-between align-items-center">
        <div>
          <h5 class="mb-1" style="color:#2e59a7;">Penyimpanan Skripsi</h5>
          <p class="text-muted mb-0">Kelola data skripsi final mahasiswa dan lihat file yang telah di-upload.</p>
        </div>
        <button class="btn btn-primary" @click="openCreate">Tambah Skripsi</button>
      </div>
    </div>
              ...
              <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                  ...
                  <tr v-if="tasks.length === 0">
                    <td colspan="9" class="text-center text-muted">Belum ada data skripsi.</td>
                  </tr>
                </tbody>
              </table>
            </div>
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{ formMode === 'create' ? 'Tambah Skripsi' : 'Edit Skripsi' }}</h5>
          ...
          <div class="modal-body">
            ...
            <div class="row g-3">
              ...
              <div class="col-md-6">
                ...
                <input class="form-control" type="text" value="skripsi" disabled />
              </div>
              <div class="col-12">
...
const loadTasks = async () => {
  try {
    const response = await fetch(`${API_BASE}/api/admin/tugas-akhir?jenis=skripsi`)
...
const saveTask = async () => {
  try {
    ...
    if (!response.ok) {
      errorMessage.value = result.message || 'Gagal menyimpan data skripsi'
      return
    }
    await loadTasks()
    closeModal()
    ...
    console.error(error)
    errorMessage.value = 'Terjadi kesalahan saat menyimpan data skripsi'
  }
}

const deleteTask = async (id: number) => {
  if (!confirm('Hapus data skripsi ini?')) return
  try {
    const response = await fetch(`${API_BASE}/api/admin/tugas-akhir/${encodeURIComponent(id)}`, { method: 'DELETE' })
    if (!response.ok) throw new Error('Gagal menghapus skripsi')
    await loadTasks()
  } catch (error) {
    console.error(error)
  }
