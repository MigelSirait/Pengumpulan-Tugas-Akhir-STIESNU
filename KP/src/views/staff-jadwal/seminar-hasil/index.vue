<template>
  <div class="container-fluid home-page-container">
    <div class="row">
      <div class="col-12">
        <h5 class="mb-3" style="color:#2e59a7;">Kelola Jadwal Seminar Hasil</h5>
                ...
                <div class="card border shadow-sm h-100">
                  <div class="card-body p-3">
                    ...
                    <ul class="list-group list-group-flush">
                      <li class="list-group-item">Draft Seminar Hasil</li>
                      <li class="list-group-item">Absensi Seminar</li>
                      <li class="list-group-item">Berita Acara Seminar</li>
                    </ul>
                  </div>
                </div>
              </div>
...
const fileFields = [
  { field: 'berkas_proposal', label: 'Draft Seminar Hasil', description: 'File laporan untuk seminar hasil' },
  { field: 'berkas_penilaian', label: 'Absensi Seminar', description: 'Dokumen absensi seminar' },
  { field: 'berkas_pengantar', label: 'Berita Acara Seminar', description: 'Berita acara seminar hasil' }
]
...
const loadSubmissions = async () => {
  ...
  statusMessage.value = ''
  try {
    const response = await fetch(`${API_BASE}/api/staf/seminar?jenis=seminar-hasil`)
    const data = await response.json()
    submissions.value = Array.isArray(data) ? data.map(mapSubmission) : []
    selectedStudent.value = submissions.value[0] || null
  } catch (error) {
    console.error(error)
    statusMessage.value = 'Gagal memuat data pengajuan seminar hasil dari server.'
