<template>
  <div class="container-fluid home-page-container">
    <div class="row">
      <div class="col-12">
        <h5 class="mb-3" style="color:#2e59a7;">Kelola Jadwal Seminar Proposal</h5>
              ...
              <div class="col-lg-4 d-flex flex-column gap-3 mb-4">
                <div class="card border shadow-sm">
                  <div class="card-body p-3">
                    ...
                    <div class="list-group" v-if="!loading && submissions.length">
                      <button
                    ...
                    <div v-if="loading" class="text-center text-muted py-4">Memuat pengajuan mahasiswa...</div>
                    <div v-else-if="!submissions.length" class="text-center text-muted py-4">
                      {{ fetchError || 'Belum ada pengajuan mahasiswa untuk jenis ini.' }}
                    </div>
                  </div>
                </div>
                ...
                <div class="card border shadow-sm h-100">
                  <div class="card-body p-3">
                    ...
                    <ul class="list-group list-group-flush">
                      <li class="list-group-item">Draft Proposal</li>
                      <li class="list-group-item">Form Penilaian Dosen Pembimbing</li>
                      <li class="list-group-item">Surat Pengantar dari Prodi</li>
                    </ul>
                  </div>
                </div>
              </div>
...
const fileFields = [
  { field: 'berkas_proposal', label: 'Draft Proposal', description: 'Draft proposal skripsi mahasiswa' },
  { field: 'berkas_penilaian', label: 'Form Penilaian Dosen', description: 'Formulir penilaian dosen pembimbing' },
  { field: 'berkas_pengantar', label: 'Surat Pengantar', description: 'Surat dari program studi' }
]
...
const loadSubmissions = async () => {
  ...
  statusMessage.value = ''
  try {
    const response = await fetch(`${API_BASE}/api/staf/seminar?jenis=seminar-proposal`)
    const data = await response.json()
    submissions.value = Array.isArray(data) ? data.map(mapSubmission) : []
    selectedStudent.value = submissions.value[0] || null
  } catch (error) {
    console.error(error)
    fetchError.value = 'Gagal memuat data pengajuan seminar proposal dari server.'
    statusMessage.value = ''
